...
def novo_epi():
    ...
    if request.method == 'POST':
        try:
            ...
            ca_existente = EPI.query.filter_by(
                codigo_ca=request.form['codigo_ca']).first()
            if ca_existente:
            ...
            if 'imagem' in request.files:
                ...
                if file and file.filename != '':
                    ...
                    upload_dir = os.path.join(
                        current_app.root_path, 'static', 'uploads', 'epis')
            ...
            novo_epi = EPI(
                ...
                data_validade_ca=datetime.strptime(
                    request.form['data_validade_ca'], '%Y-%m-%d').date(),
    ...
    if request.method == 'POST':
        try:
            ...
            ca_existente = EPI.query.filter(
                and_(EPI.codigo_ca ==
                     request.form['codigo_ca'], EPI.id != epi_id)
            ).first()
            if ca_existente:
            ...
            epi.data_validade_ca = datetime.strptime(
                request.form['data_validade_ca'], '%Y-%m-%d').date()
...
def api_epis():
    ...
    try:
        ...
        if request.args.get('busca'):
            ...
            query = query.filter(
                or_(EPI.nome.ilike(busca), EPI.codigo_ca.ilike(
                    busca), EPI.fabricante.ilike(busca))
            )
    ...
    try:
        ...
        declaracao = DeclaracaoEPI(
            ...
            data_declaracao=datetime.strptime(
                data['data_declaracao'], '%Y-%m-%d').date(),
        ...
        for item in data['epis']:
            if item['acao'] == 'entregar':
                ...
                if epi:
                    epi.estoque_atual = max(
                        0, epi.estoque_atual - item.get('quantidade', 1))
                ...
                if entrega_existente:
                    ...
                    entrega_existente.motivo_devolucao = item.get(
                        'motivo_devolucao', '')
...
def gerar_pdf_declaracao(declaracao_id):
    ...
    try:
        ...
        entregas = EntregaEPI.query.filter_by(
            declaracao_id=declaracao_id).all()
        ...
        # Cabeçalho
        story.append(
            Paragraph("ALTA TELAS REDES DE PROTEÇÃO - LTDA", title_style))
        story.append(
            Paragraph("DECLARAÇÃO DE RECEBIMENTO DE EPI", styles['Heading2']))
        ...
        # Tabela de EPIs
        epi_data = [['DATA', 'RD', 'DESCRIÇÃO DO EPI',
        ...
        for entrega in entregas:
            epi_data.append([
                ...
                entrega.data_devolucao.strftime(
                    '%d/%m/%Y') if entrega.data_devolucao else ''
            ])
        ...
        for config in data['alertas']:
            if config.get('id'):
                ...
                if alerta:
                    ...
                    alerta.emails_destino = json.dumps(
                        config.get('emails', []))
