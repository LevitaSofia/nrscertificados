...
# Criação do blueprint para o módulo EPI
epi_bp = Blueprint('epi', __name__, 
                   template_folder='templates/epi',
...
def inserir_configuracoes_padrao():
    ...
    configuracoes_padrao = [
        ...
        ('dias_alerta_vencimento', '30', 'Dias de antecedência para alertar vencimento CA', 'integer'),
        ('frequencia_verificacao', 'diaria', 'Frequência de verificação de alertas', 'string'),
        ('alerta_vencimento_ca', 'true', 'Alertar vencimento de CA', 'boolean'),
        ('alerta_estoque_baixo', 'true', 'Alertar estoque baixo', 'boolean'),
        ('alerta_devolucao_atraso', 'true', 'Alertar devolução em atraso', 'boolean'),
        ...
        ('emails_destinatarios', '', 'E-mails destinatários (separados por vírgula)', 'string'),
        ('email_ssl', 'true', 'Usar SSL/TLS', 'boolean'),
        
        # API Externa
        ('api_ca_ativa', 'false', 'API de validação de CA ativa', 'boolean'),
        ('api_ca_url', 'https://api.trabalho.gov.br/ca/validar', 'URL da API de validação CA', 'string'),
...
def configurar_jobs_automaticos():
    ...
    def job_alertas():
        ...
        try:
            ...
            print(f"✅ Alertas automáticos processados - {time.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
    ...
    def job_backup():
        ...
        try:
            ...
            print(f"✅ Backup automático executado - {time.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
    ...
    def job_limpeza():
        ...
        try:
            ...
            print(f"✅ Limpeza automática executada - {time.strftime('%Y-%m-%d %H:%M:%S')}")
        except Exception as e:
...
def adicionar_context_processors(app):
    ...
    def epi_context():
        ...
        try:
            ...
            alertas_pendentes = AlertaEPI.query.filter_by(status='ativo').count()
            
            # Configurações importantes
            empresa_nome = ConfiguracaoEPI.get_config('empresa_nome', 'Sistema de Gestão')
